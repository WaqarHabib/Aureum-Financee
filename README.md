...
**Private Wealth Horizon** is a refined, browser-based cash-flow studio for projecting your future balance with quiet precision. Aureum lets you record a starting balance, schedule income and obligations, visualise the resulting timeline, and export an editorial-grade private statement that prints to PDF.
...
Aureum is a static single-page application and deploys cleanly to Vercel, Netlify, Cloudflare Pages or any static host. A `vercel.json` is included with an SPA rewrite rule so client-side routes resolve correctly.
